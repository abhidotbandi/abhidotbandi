    ...
    <SectionShell id="finale" align="center" className="max-w-2xl text-center">
      ...
      <p className="text-slate-300">
        <span className="font-mono">
          {badges.size}/4 badges earned · {xp} XP
        </span>
        . Base Power is making the case that the fastest, cheapest way to make the grid more
        reliable isn&apos;t a new power plant or a new transmission line — it&apos;s a battery in
        every garage, coordinated as one.
      </p>
          ...
          <div
            ...
            className={`rounded-md border p-2.5 text-xs ${
              badges.has(id as keyof typeof BADGES)
                ? "border-livewire/50 bg-livewire/10 text-livewire"
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
        <a
          ...
          className="rounded-md bg-livewire px-5 py-2.5 text-sm font-semibold text-slate-950 hover:bg-livewire/80"
        >
          See Base Power
        </a>
        <button
          onClick={handleReplay}
          className="rounded-md border border-white/15 px-5 py-2.5 text-sm font-semibold text-slate-200 hover:border-white/30"
        >
          Replay the journey
        </button>
      </div>
